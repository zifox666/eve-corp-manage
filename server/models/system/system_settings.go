...
func (m *SystemSetting) Get(configName string) (result string, err error) {
	...
	if err := global.Db.Model(m).Select("ConfigValue").First(&res, "config_name=?", configName).Error; err != nil {
		return result, err
	}
...
func (m *SystemSetting) Set(configName string, configValue interface{}) error {
	...
	db := global.Db.Model(m).First(&findRes, "config_name=?", configName)
	...
	if db.RowsAffected == 0 {
		// 添加
		if err := global.Db.Model(m).Create(&SystemSetting{ConfigName: configName, ConfigValue: value}).Error; err != nil {
			return err
		}

	} else {
		// 修改
		if err := global.Db.Model(m).Where("id=?", findRes.ID).Update("config_value", value).Error; err != nil {
			return err
		}
	}
	return nil
